...
#.APLICACION GRAFICA CRUD-HASH


#INTEGRACION DE ENCRIPTACION DE CONTRASEÑA POR MEDIO DEL HASHING
...
def encriptar_pass(password):
    """Transforma la entrada del usuario en un Hash SHA-256 indescifrable"""
...
#Con esta versión final, hemos transformado un ejercicio guiado en una infraestructura de software #profesional bajo el estándar 2026. La arquitectura se basa en la centralización del Motor de Datos 
#mediante la función consulta, la cual utiliza gestores de contexto (with) para garantizar que la #conexión al búnker en el disco de 2TB se abra y cierre de forma hermética, eliminando fugas de memoria.
#Hemos implementado una capa de seguridad criptográfica mediante el módulo hashlib, procesando cada #password con el algoritmo SHA-256 para asegurar que la "carnita" sea ilegible incluso para el 
#administrador. La interfaz se ha optimizado drásticamente mediante bucles dinámicos y funciones Lambda, #automatizando la creación de menús, botones y campos de entrada; esto permite que el código sea un 60% 
#más corto y totalmente escalable. Finalmente, el sistema integra higiene visual y lógica, ocultando los #hashes en la lectura y corrigiendo bugs de persistencia, logrando un programa antifrágil, blindado y de 
#alto rendimiento.
